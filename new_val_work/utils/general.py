...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    loss = lbox + lobj + lcls
    # print('iou_loss:{},conf_loss:{},cls_loss{}'.format(lbox, lobj, lcls))
    # print('loss:', loss)
...
def non_max_suppression(prediction, conf_thres=0.5, iou_thres=0.6,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        # x[:, 5:109] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            # i, j = (x[:, 5:109] > conf_thres).nonzero(as_tuple=False).T
            i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T

            # x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
            # add by zlf at 20201022
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(),x[i,4,None],
                           x[i, j + 5, None]/x[i,4,None]), 1)
...
# 针对筛选logo的nms
def non_max_suppression_for_logo(prediction, conf_thres=0.5, iou_thres=0.6,
                        merge=False, classes=None, agnostic=False,
                        logo_conf = None):
    """Performs Non-Maximum Suppression (NMS) on inference results
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            if logo_conf:
                ...
                res1[:,2:] = res2[:,2:]
                res = res1
                i,j = res.nonzero(as_tuple=False).T
            # 20201120
...
def strip_optimizer(f='weights/best-12.pt', s=''):  # from utils.general import *; strip_optimizer()
...
def print_mutation(hyp, results, yaml_file='hyp_evolved.yaml', bucket=''):
    # Print mutation results to evolve.txt (for use with train_zlf.py --evolve)
