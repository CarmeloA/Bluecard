        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with open(results_file, 'r') as f:  # create checkpoint
                ckpt = {'epoch': epoch,
                        ...
                        'optimizer': None}
            if i == 0 or i % 400 == 0:
                # torch.save(model.state_dict(),'{}_{}.pt'.format(epoch,i))
                # print('mloss:',mloss)
                # torch.save(ckpt,last)
                torch.save(ckpt, wdir / 'ckpt_model_{}_{}_{}.pt'.format(epoch,i,round((mloss.sum()/2.).item(),5)))
                input1 = torch.zeros((1,3,192,320)).cuda()
                model = torch.jit.trace(ema.ema,input1)
                torch.save(model.state_dict(), wdir / 'model_{}_{}_{}.pt'.format(epoch,i,round((mloss.sum()/2.).item(),5)))
            del ckpt
            # end batch ------------------------------------------------------------------------------------------------
